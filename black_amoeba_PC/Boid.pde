...
class Boid extends Flagellum {
  ...
  void display() {
    ...
    //translate(location.x + mouseX, location.y + mouseY);
  ...
  void borders() {
    if (location.x < -skin.width) location.x = 300;
    if (location.x > 300 + skin.width) location.x = 0;
    if (location.y < -skin.width) location.y = 300;
    if (location.y > 300 + skin.width) location.y = 0;
  }
}
