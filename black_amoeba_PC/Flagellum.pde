...
class Flagellum {
  ...
  Flagellum(String _skin) {
    skin = loadImage("skin-001.png");


    //     skin.filter(Blur,1)
    // random image resize
    float scalar = random(1, 1);
    int valv = int(random(1, 1));
    fastBlur(skin, valv);
  ...
  void move() {
    ...
    //node[0].x = cos(radians(theta - mouseX));
    //node[0].y = sin(radians(theta) - mouseY);
...
void vertexi(float a, float b, float c, float d) {
  fill(a, b, c, 255-b);
  vertex(a, b);
}